...
Kể như một chuỗi chẩn đoán. Dummy 0.069 là floor. TBM ban đầu 0.161. De novo giúp target không template lên 0.212. Reproduce top-1 đạt 0.297 và cho thấy search là bottleneck. Khi thêm composite search, score lên 0.307. Đây là bằng chứng thực nghiệm rằng candidate generation quan trọng hơn optimizer trong regime hiện tại.
...
Máy mới đã có env `rna-fold`, CUDA thấy RTX 3060 Ti, MMseqs và test pass. Kaggle API token hoạt động và tài khoản hiện chưa có submission. Hai notebook cũ tồn tại nhưng output session API không còn nên không đoán version để nộp. Data 61 GiB đã tải và audit đủ CSV, MSA, MSA_v2 cùng 8.670 CIF. WSL memory setting cần `wsl --shutdown` mới có hiệu lực.

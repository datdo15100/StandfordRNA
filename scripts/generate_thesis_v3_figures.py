...
OUT = ROOT / "reports" / "thesis_v3" / "figures"
...
def setup() -> None:
    ...
    if times_path.exists():
        ...
        font_family = font_manager.FontProperties(fname=times_path).get_name()
    else:
...
def leaderboard_results() -> None:
    ...
    public = [np.nan, np.nan, np.nan, 0.60084, 0.62809]
    private = [0.57631, 0.57773, 0.59298, 0.60175, 0.61390]
    ...
    for i, value in enumerate(private):
        ax.text(i + width / 2, value + 0.002, f"{value:.3f}", ha="center", fontsize=8, color=NAVY, weight="bold")
    for i, value in enumerate(public):
        if np.isfinite(value):
            ax.text(i - width / 2, value + 0.002, f"{value:.3f}", ha="center", fontsize=8, color=TEAL, weight="bold")
    ax.annotate(
        "+0.012 vs thesis TBM\n+0.036 vs winning team",
        xy=(4.16, 0.6139),
        xytext=(3.15, 0.633),
...
def leakage_diagnostic() -> None:
    ...
    ax.grid(axis="y", alpha=0.2)
    for i, value in enumerate(values):
        ax.text(i, value + 0.025, f"{value:.3f}", ha="center", weight="bold", color=colors[i])
